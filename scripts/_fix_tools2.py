"""One-shot script to fully rewrite tools.py with correct UTF-8."""
import pathlib
...
print("tools.py fully rewritten OK")

# Verify no garbled chars remain
with open(tools_path, "r", encoding="utf-8") as f:
    check = f.read()
garbled = [c for c in ["ÅŸ", "Ä±", "Ã¶", "ÃŒ", "Ã§"] if c in check]
if garbled:
    print(f"WARNING: Still has garbled: {garbled}")
else:
    print("No garbled chars found — clean!")
assert "`contains` kullan" in check
assert "OR kullan" in check
